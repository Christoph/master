...
public class QueryManager {
  ...
  public List<Tag> getAll() throws SQLException {
  	...
  	while(result.next()) {    
  		data.add(new Tag(result.getInt("TTID"), result.getString("TagName").toLowerCase().replace('-', ' '), result.getInt("Playcount"), result.getInt("TagID"), result.getInt("TagWeight"), result.getInt("SongID"),result.getString("SongName").toLowerCase(), result.getInt("Listeners"),result.getInt("ArtistID")));
  	}
